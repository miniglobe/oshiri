...
class NWModel(object):
  ...
  def __init__(self, source_vocab_size, target_vocab_size, buckets, size,
    ...
    with tf.device('/gpu:0'):
        ...
        single_cell = core_rnn_cell.BasicLSTMCell(size)
        cell = single_cell
        if num_layers > 1:
  ...
  def get_batch(self, data, bucket_id):
    ...
    for _ in xrange(self.batch_size):
      ...
      decoder_inputs.append([input_reader.GO_ID] + decoder_input +
                            [input_reader.PAD_ID] * decoder_pad_size)
    ...
    for length_idx in xrange(decoder_size):
      ...
      for batch_idx in xrange(self.batch_size):
        ...
        if length_idx == decoder_size - 1 or target == input_reader.PAD_ID:
